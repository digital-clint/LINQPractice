...
namespace PracticeProblemsLINQ
{
    public static class LinqProblems
    {
        ...
        {
            //code
            var wordResults = words.Where(w => w.Contains("th"))
                                   .ToList();

            Console.WriteLine("Below are the words that contain 'th' based on the list: ");
        ...
        {
            //code
            var copyCatList = names.Distinct()
                                   .ToList();

            Console.WriteLine("Below are the list of names contained in the list without duplicates: ");
            foreach (var name in copyCatList)
            {
        ...
        public static Customer RunProblem3(List<Customer> customers)
        {
            ...
            var foundName = from c in customers
                            where c.FirstName == "Mike"
                            select c;
            

            
            Console.WriteLine("We've found Mike!");
            foreach (var item in foundName)
            {
               
                Console.WriteLine($"His name is {item.FirstName} {item.LastName} and his identification number is: {item.Id}");


            }

            //return
            return foundName.FirstOrDefault();
        ...
        {
            //code
            var foundID = from c in customers
                            where c.Id == 3
                            select c;


            foreach (var item in foundID)
            {

                Console.WriteLine($"Customer who has an id of 3 is:  {item.FirstName} {item.LastName}");
